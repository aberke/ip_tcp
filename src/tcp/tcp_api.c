...
void* tcp_api_read_entry(void* _args){
	...
	int ret = tcp_api_read(args->node, args->socket, (unsigned char*)args->buffer, args->num);
	if(args->boolean){
		...
		while(ret < args->num){
			...
			read = tcp_api_read(args->node, args->socket, (unsigned char*)(args->buffer)+ret, (args->num)-ret);
			if(read < 0){
				_return(args, read);
				return NULL;
			}
